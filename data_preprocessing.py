...
def compute_class_weights(y_encoded,le, num_classes=5):
   ...
   weight_dict = {int(cls): weight for cls, weight in zip(classes,weights)}
   print("\nClass weights:")
...
def prepare_data(data_path='data/mitdb', download=False):
    ...
    class_weights = compute_class_weights(y_train_enc,le)
